...
function logEvent(type) {
    if (state.activeSession) {
        state.activeSession.events.push({ type, timestamp: Date.now() });
    }
}
...
function updateTimerDisplay(ms) {
    ...
    document.title = `${m}:${s.toString().padStart(2, '0')} - Focus Pulse`;
}
...
function saveData() {
    ...
    localStorage.setItem('focusPulseData', JSON.stringify(data));
}

function loadData() {
    const json = localStorage.getItem('focusPulseData');
    if (json) {
...
function resetAllData() {
    if (confirm('Create slate clean? This will delete all history.')) {
        localStorage.removeItem('focusPulseData');
        state.sessions = [];
        state.activeSession = null;
        renderHistory();
        switchView('dashboard');
